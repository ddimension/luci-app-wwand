...
return view.extend({
	...
	// Per-SIM overrides (config wwand_sim in /etc/config/network): a PIN/APN
	// bundle matched at runtime by ICCID, independent of the modem (a wwand_sim
	// need not be bound to a modem — leave Modem empty). Used for dual-SIM or
	// eUICC profiles with different PINs. The Add form prefills the ICCID from the
	// active card (read via modem_sim_slots).
	renderSimOverrides: function(data) {
		var self = this;
		var out = [ E('h4', {}, _('Per-SIM overrides')) ];

		var sims = uci.sections('network', 'wwand_sim');
		var active = (data.slots || []).filter(function(s) { return s.active; })[0];
		var prefill = (active && active.iccid) || '';

		var rows = sims.map(function(s) {
			return E('tr', { 'class': 'tr' }, [
				E('td', { 'class': 'td' }, s.iccid || '—'),
				E('td', { 'class': 'td' }, s.pincode ? '••••' : '—'),
				E('td', { 'class': 'td' }, s.apn || '—'),
				E('td', { 'class': 'td' }, s.modem || _('(any)')),
				E('td', { 'class': 'td' }, E('button', {
					'class': 'btn cbi-button cbi-button-remove',
					'click': ui.createHandlerFn(self, function() {
						if (!confirm(_('Delete the SIM override for %s?').format(s.iccid || '?')))
							return;
		...
		out.push(E('table', { 'class': 'table' }, [
			E('tr', { 'class': 'tr table-titles' }, [
				E('th', { 'class': 'th' }, 'ICCID'), E('th', { 'class': 'th' }, _('PIN')),
				E('th', { 'class': 'th' }, 'APN'), E('th', { 'class': 'th' }, _('Modem')),
				E('th', { 'class': 'th', 'style': 'width:1%' }, '') ]),
		].concat(rows.length ? rows : [ E('tr', { 'class': 'tr' },
			[ E('td', { 'class': 'td', 'colspan': 5 }, E('em', {}, _('none'))) ]) ])));

		var iccidIn = E('input', { 'type': 'text', 'class': 'cbi-input-text',
			'style': 'width:16em', 'placeholder': 'ICCID', 'value': prefill });
		var pinIn = E('input', { 'type': 'text', 'class': 'cbi-input-text',
			'style': 'width:7em', 'placeholder': _('PIN') });
		var apnIn = E('input', { 'type': 'text', 'class': 'cbi-input-text',
			'style': 'width:12em', 'placeholder': 'APN' });
		var addBtn = E('button', { 'class': 'btn cbi-button cbi-button-add', 'style': 'margin-left:8px',
			'click': ui.createHandlerFn(self, function() {
				var iccid = (iccidIn.value || '').trim();
				if (!iccid) {
					ui.addNotification(null, E('p', _('Enter the SIM ICCID.')), 'warning');
					return;
				}
				var sid = uci.add('network', 'wwand_sim');
				uci.set('network', sid, 'iccid', iccid);
				if ((pinIn.value || '').trim()) uci.set('network', sid, 'pincode', pinIn.value.trim());
				if ((apnIn.value || '').trim()) uci.set('network', sid, 'apn', apnIn.value.trim());
				return uci.save().then(function() { return uci.apply(); })
					.then(function() { window.location.reload(); });
			}) }, _('Add SIM override'));

		out.push(E('div', { 'style': 'margin-top:6px' }, [ iccidIn, ' ', pinIn, ' ', apnIn, addBtn ]));
		out.push(E('p', { 'style': 'color:#666;font-size:90%' },
			_('A per-SIM override matches by ICCID and applies to whichever modem holds that card (leave Modem empty). The ICCID is prefilled from the active card.')));
		return out;
	},
